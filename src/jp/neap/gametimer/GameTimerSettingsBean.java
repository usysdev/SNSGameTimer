...
/*
		if (laterMinute > 0) {
			return notifyText + " (" + laterMinute + "分後)";
		}
		if (laterHourMinuteHour > 0 || laterHourMinuteMinute > 0) {
			return notifyText + " (" + laterHourMinuteHour + "時間" + laterHourMinuteMinute + "分後)";
		}
		if (atTimeHour > 0 || atTimeMinute > 0) {
			return notifyText + " (" + atTimeHour + "時" + atTimeMinute + "分に)";
		}
*/
		if ( notifyDateTime > 0 ) {
			final Calendar c = Calendar.getInstance();
	...
	public String getNotifyDateExpression() {
		if ( laterMinute > 0 ) {
			return laterMinute + "分後";
		}
		if ( (laterHourMinuteHour > 0) ||
			 ((laterHourMinuteHour == 0) && (laterHourMinuteMinute > 0)) ) {
			return laterHourMinuteHour + "時間" + laterHourMinuteMinute + "分後";
		}
		if ( atTimeHour > 0 ) {
